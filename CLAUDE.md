...
2. Regenerate minified version:
   ...
   ```
3. Bump the CSS cache version parameter (`?v=N`) in all HTML files and in `build.js` (search globally for `styles.min.css?v=`). Every page references this, including the listing page template in build.js. **Current version: `v=50`.**
...
- **Spacing scale (`--page-x` / `--section-y`)**: every page-edge horizontal padding and major vertical section gap reads from two CSS custom properties declared on `:root` in `css/styles.css`: `--page-x` (horizontal page padding) and `--section-y` (bottom padding on major sections). Desktop defaults are `--page-x: 24px` and `--section-y: 72px`. The `@media (max-width: 768px)` block overrides these to `22px` / `80px` (mobile gets *more* vertical rhythm than desktop because stacked content reads as cramped otherwise), and `@media (max-width: 400px)` further tightens horizontal padding to `20px` so 320–400px viewports still get usable content width. When you add a new full-width content block, **use `padding: 0 var(--page-x)`** instead of hardcoding `24px` so the new section participates in the scale automatically. Never hardcode `16px` or `24px` for page-edge padding — that bypasses the responsive scale and reintroduces the edge-to-edge cramping this system was designed to prevent.
