...
| `listings/[slug]/index.html` (sold stub) | `Product` (**no `offers` block** — `brand` / `image` / `itemCondition` / `sku` only; see §6.3), `BreadcrumbList`, `ItemList` of `ImageObject` (per-photo gallery) |
...
(`robots: index, follow`) and keeps a coherent `Product` record (the visible
"This piece has sold." overlay conveys the sold state to crawlers).

**No `offers` block on a sold stub.** The stub `Product` omits `offers`
entirely — no `availability`, no `price`. The reasoning is a forced choice:
Google treats `price` as **required inside any `Offer`**, so an `Offer`
without a price is a *critical* validation error, while an `Offer` *with* a
price both violates the "markup reflects visible content" rule (the page
shows no price) and leaks a seller-side negotiation anchor to AI/answer
crawlers and scrapers — undercutting the acquisition margins the business
depends on (the same reason sell pages publish no per-piece figures — §5.13,
§10.12). Because a sold item is ineligible for product rich results either
way, the `Offer` carried zero discovery value, so the clean resolution is to
drop it: `brand`, `image` + the `ImageObject` layer (§5.16), and
`itemCondition` carry all the real signal, and the page validates with only a
harmless non-critical "missing offers" note. (This supersedes the earlier
`availability: SoldOut` approach — `SoldOut` lived inside the `Offer` that was
removed; it is no longer emitted. The `SoldOut`-not-`OutOfStock` distinction
in §10.1 is retained only as a guard against reintroducing a priced `Offer`
with the wrong availability value.)
...
5. **Give the piece a sold-stub listing page** (rationale in §6.3) — never
   ...
   - Run `node scripts/gen-sold-stub.js <slug>`. It reads the piece's photos
     ...
     reference stubs: `Product` schema (keeps `sku`, `name`, `description`,
     `brand`, `image`, `itemCondition`; **no `offers` block at all** — see §6.3
     for why; consequently no `availability` / `price` / `priceValidUntil` /
     return / shipping), `BreadcrumbList`, per-photo
...
**DO NOT USE FOR NEW IMPLEMENTATION.** Sold stubs now omit the `offers` block
entirely (no `availability` is emitted — see §6.3), so this rule is mostly a
historical guard. *If* an availability value is ever reintroduced on a sold
piece, it must be `SoldOut`, never `OutOfStock`: `OutOfStock` implies the item
may return; `SoldOut` correctly signals a permanent, one-of-one sale. Note that
reintroducing any `Offer` would also force a `price` back onto the page
(Google requires it), which §6.3 deliberately avoids — so the preferred state
remains *no `offers` at all*.
