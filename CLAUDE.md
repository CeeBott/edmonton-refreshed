...
| `sell/[slug]-edmonton/index.html` | `BreadcrumbList`, `Service` (with `dateModified`), `FAQPage`, `ItemList` of `ImageObject` items (one per displayed sold card, generated by `build.js` between `LANDING_SOLD_SCHEMA_*` markers — see §5.16) when the page displays a `.sell-landing-sold-grid` |
