...
2. Regenerate minified version:
   ...
   ```
3. Bump the CSS cache version parameter (`?v=N`) in all HTML files and in `build.js` (search globally for `styles.min.css?v=`). Every page references this, including the listing page template in build.js. **Current version: `v=49`.**
...
- **Global footer**: Every page carries the same `<footer class="site-footer">` block — a centered brand statement, a three-column taxonomy (Sell By Piece / Sell By Situation / Sell By Brand) linking to the 18 sell-side landing pages, and a quiet utility row (Recently Sold, Guides, About, Privacy Policy) alongside the copyright and tagline. Styling lives under `.site-footer` / `.footer-*` in `css/styles.css` — editorial, muted, typography-led. The same markup is duplicated in every HTML page **and** in the listing-page template inside `build.js`. When you add a new sell-side landing page, update **every** footer copy (manual pages + the build.js template) so the taxonomy stays in sync — the same way the nav dropdown is maintained. Mobile collapses the three columns to a single stacked column with padded link blocks for thumb-friendly tap targets. Sold-stub listing pages also use the global footer (the bespoke `.footer` markup the canonical stub previously carried has been retired).
