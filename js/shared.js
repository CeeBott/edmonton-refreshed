...
function buildCarousel(images, alt) {
  ...
  var imgsHtml = images.map(function(src, i) {
    ...
    return '<picture><source type="image/webp" srcset="' + webpSrcset + '" sizes="(max-width: 768px) 100vw, 530px"><img src="' + src + '" srcset="' + srcset + '" sizes="(max-width: 768px) 100vw, 530px" alt="' + alt + ' — photo ' + (i+1) + '" loading="' + loadAttr + '"' + fetchPri + dims + ' draggable="false"></picture>';
  }).join('');
